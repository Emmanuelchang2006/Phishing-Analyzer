...
class RiskEngine:
    ...
    def compute(self, ctx: RiskContext) -> RiskScore:
        ...
        # TOR exit nodes always warrant at least SUSPICIOUS regardless of other signals
        tor_detected = any(
        ...
        # Confidence: how many data sources returned real results vs errors
        active_sources = sum(
            1 for r in ctx.threat_intel if r.error is None
        )
        ...
        total_possible = len(ctx.threat_intel) + (1 if ctx.whois else 0) + (1 if ctx.email_auth else 0)
        total_successful = active_sources + (1 if has_whois else 0) + (1 if has_email_auth else 0)
        ...
        for result in results:
            ...
            if "virustotal" in result.source:
                ...
                if malicious >= 5:
                    score += 40
                    factors.append(
                        f"{source_label}: {malicious}/{total} engines flagged as MALICIOUS"
                    )
                elif malicious >= 1:
                    score += 25
                    factors.append(
                    ...
                    score += 15
                    factors.append(
                        f"{source_label}: {suspicious}/{total} engines flagged as suspicious"
                    )

                # VT community reputation is independent of engine detections
                if reputation is not None and isinstance(reputation, (int, float)):
                    rep = int(reputation)
                    if rep <= -50:
                        score += 15
                        factors.append(
                            f"{source_label}: very negative community reputation ({rep:+d})"
                        )
                    elif rep <= -10:
                        score += 8
                        factors.append(
                            f"{source_label}: negative community reputation ({rep:+d})"
                        )
                ...
                if confidence >= 80:
                    score += 30
                    factors.append(
                        f"AbuseIPDB: {confidence}% abuse confidence ({reports} reports)"
                    )
                elif confidence >= 25:
                    score += 15
                    factors.append(
                    ...
                    score += 8
                    factors.append(
                        f"AbuseIPDB: {confidence}% abuse confidence ({reports} reports) — low confidence"
                    )
